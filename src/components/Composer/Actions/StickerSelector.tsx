...
import { ContentTypeSticker } from "@codecs/Sticker"
...
const StickerSelector = ({
    ...
    const { setShowAddStickerModal } = useModalStore()
    const onSelectSticker = async (sticker: any) => {
        onClose?.()
        setSendingXmtpMessage(ContentTypeSticker)
        ...
        if (replyingMessage) {
            const reply: Reply = {
                ...
                messageContentTypeId: ContentTypeSticker.typeId,
            }

            await sendXmtpMessage(reply, ContentTypeReply, sticker?.url)
        } else {
    ...
    return (
        <div className="bg-[#FFFFFF] border rounded-lg w-[350px] h-[456px] overflow-hidden relative">
            <div className="flex justify-between items-center pt-3.5 pb-0.5 px-5 divider mb-1">
                ...
                <IconButton
                    aria-label="delete"
                    className="p-1 text-white rounded-full hover:bg-badgeRed1 transition-all from-text_gradient1 to-text_gradient2 bg-gradient-to-br"
                    onClick={() => {
            ...
            <UnifiedComponent
                ...
                stickerClick={onSelectSticker}
                loadingColor="#FE06C7"
                menu={{
                    selectedLine: "2px solid #FE06C7",
                    bottomLine: "0px",
                }}
            />
        </div>
