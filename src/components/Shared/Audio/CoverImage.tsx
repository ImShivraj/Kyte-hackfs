...
import { COVER, ERROR_MESSAGE } from "@constants/index"
...
const CoverImage: FC<Props> = ({
    ...
    const onError = (error: any) => {
        showNotification({
            ...
            message: error?.data?.message ?? error?.message ?? ERROR_MESSAGE,
        })
        setLoading(false)
    }
            ...
            {isNew && (
                <label
                    className={clsx(
                        ...
                        "absolute top-0 grid md:w-40 md:h-40 h-24 w-24 bg-gray-100 cursor-pointer place-items-center group-hover:visible backdrop-blur-lg"
                    )}
                >
                    {loading && !cover ? (
                        <Spinner size="sm" />
                    ) : (
                        <div className="text-sm text-black flex flex-col opacity-60 items-center">
