...
import { usePublicationStore } from "@store/publicaiton"
import { object, string } from "zod"

import CoverImage from "./CoverImage"
import Player from "./Player"
import { KytePublication } from "src/types"
import { Attribute } from "@lens/generated"
...
const Audio: FC<Props> = ({ src, isNew = false, publication, txn }) => {
    ...
    return (
        <div className="border px-3.5 pt-3.5 md:p-0 bg-brand-500 overflow-hidden rounded-xl">
                        ...
                        <div className="flex items-center space-x-2.5 w-full truncate">
                            <button type="button" onClick={handlePlayPause}>
                                {playing && !playerRef.current?.plyr.paused ? (
                                    <PlayCircle className="w-[50px] h-[50px] text-gray-100 hover:text-white" />
                                ) : (
                                    <PlayCircle className="w-[50px] h-[50px] text-gray-100 hover:text-white" />
                                )}
                            </button>
                            <div className="w-full pr-3 truncate">
                                {isNew && !txn ? (
                                    ...
                                    <>
                                        <h5 className="text-lg text-white truncate">
                                            {publication?.metadata.name ??
                                                txn.title}
                                        </h5>
                                        <h6 className="text-white/70 truncate">
                                            {txn?.author ??
                                                getAttributeFromTrait(
